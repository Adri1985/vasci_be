...
// Actualizar la cantidad de un producto en la suscripción
router.post('/update', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Encontrar el producto en la suscripción
    const product = user.subscription.find((item) => item.id === id);
    if (!product) {
      return res.status(404).json({ message: 'Producto no encontrado en la suscripción' });
    }
    ...
    res.status(200).json({ message: 'Producto actualizado', subscription: user.subscription });
  } catch (error) {
